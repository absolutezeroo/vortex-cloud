...
Orleans is configured in `Vortex.Main/Extensions/HostApplicationBuilderExtensions.cs` and invoked from `Vortex.Main/Program.cs`.
...
- `Vortex.Primitives/Orleans/OrleansStorageNames.cs`
- `Vortex.Primitives/Orleans/OrleansStreamProviders.cs`
- `Vortex.Primitives/Orleans/OrleansStateNames.cs`
- `Vortex.Primitives/Orleans/OrleansStreamNames.cs`

## Storage and State in This Repo
Persistent grain state is defined with `[PersistentState(...)]` and named via shared constants.

Example from `Vortex.Players/Grains/PlayerPresenceGrain.cs`:
...
`Vortex.PacketHandlers/Catalog/PurchaseFromCatalogMessageHandler.cs`:
...
`Vortex.Players/Grains/PlayerPresenceGrain.cs`:
...
1. Define grain interface and contracts in `Vortex.Primitives` (or the relevant shared abstraction project).
2. Implement grain in the owning module (for example `Vortex.Players/Grains`, `Vortex.Rooms/Grains`, etc.).
...
- Hardcoding stream/storage names instead of using `Vortex.Primitives/Orleans` constants.

## File Map
- Host bootstrap: `Vortex.Main/Program.cs`
- Orleans host config: `Vortex.Main/Extensions/HostApplicationBuilderExtensions.cs`
- State/storage/stream constants: `Vortex.Primitives/Orleans/*.cs`
- Player presence grain: `Vortex.Players/Grains/PlayerPresenceGrain.cs`
- Catalog handler grain usage: `Vortex.PacketHandlers/Catalog/PurchaseFromCatalogMessageHandler.cs`
- Plugin lifecycle integration: `Vortex.Plugins/PluginManager.cs`
