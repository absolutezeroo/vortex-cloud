...
- `Vortex.Specs/`, `Vortex.Specs.Cli/`
  - Development-time only, referenced by nothing at runtime. Reads this repository's own C# with
    Roslyn plus the sibling client and reference-emulator checkouts, and generates the behavioural
...
## Habbo protocol behaviour
- `docs/habbo-specs/` is the reference for what a packet carries and what the server does with it,
  with the confidence of each claim attached. Generated by `Vortex.Specs.Cli`, never hand-authored.
- Emulator behaviour recorded there is evidence about this codebase, not a statement about Habbo.
  Official behaviour nobody has captured stays `unknown`.
- Header ids are per-revision and live only in `docs/habbo-specs/revisions/`; behavioural specs name
  packets symbolically.
