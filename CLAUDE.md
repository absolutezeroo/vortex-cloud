...
| `scripts/hooks/check-variable-capabilities.mjs` | FastCheck | a wired variable declaring a `WiredVariableFlags` capability nothing implements — every picker in the wired editor filters on one flag, so the box is offered, picked, saved, and silent. Catches a named-but-unimplemented capability only: a flag copied from another variable at runtime (the echo box) and a flag that is true over a missing store (the room, context and shared variables) are both invisible to it |
| `scripts/hooks/check-wire-conflicts.mjs` | QualityGate | two ratchets, one baseline each. (a) a NEW field-count disagreement with the official client; the known ones are baselined. (b) a NEW packet where our parser reads **fewer** values than the client hands its composer — the case where the client scanner failed to resolve the composer body, so (a) is structurally blind to it and the call-site arity in the spec evidence is the only witness. (a) needs the client sources beside the repo and says so instead of passing quietly; (b) reads only committed spec evidence, so it runs everywhere, CI included |
