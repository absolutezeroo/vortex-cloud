...
/// <summary>Fires when a contract transaction was cancelled, refused or timed out. The player the transaction was waiting on is the triggering user, so
...
) : FurnitureWiredTriggerLogic(grainFactory, stuffDataFactory, ctx)
{
    ...
    public override List<Type> SupportedEventTypes { get; } = [typeof(WiredTransactionFailedEvent)];

    public override Task<bool> CanTriggerAsync(IWiredProcessingContext ctx, CancellationToken ct)
    {
        if (ctx.Event is not WiredTransactionFailedEvent evt)
        {
