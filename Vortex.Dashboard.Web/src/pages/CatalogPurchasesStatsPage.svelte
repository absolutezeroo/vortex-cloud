<script lang="ts">
  ...
  import type { CatalogPurchaseStats } from '../lib/apiTypes';
  ...
  let error = $state('');
  let data = $state<CatalogPurchaseStats | null>(null);
  ...
  async function refresh() {
    ...
    try {
      data = await apiGet<CatalogPurchaseStats>(`/api/v1/catalog/purchases/stats?${params}`);
    } catch (err) {
...
<section class="panel">
  <PageHeader title={$t('catalogPurchases.title')} description={$t('catalogPurchases.description')}>
    {#snippet actions()}
