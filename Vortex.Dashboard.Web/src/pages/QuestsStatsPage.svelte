<script>
  ...
  async function refresh() {
    ...
    try {
      data = await apiGet(`/api/quests/stats?${params}`);
    } catch (err) {
...
<section class="panel">
  ...
  {:else if forbidden}
    <AccessDeniedNotice message={$t('questsStats.accessDenied')} />
  {:else if error}
...
{#if data}
  ...
  <div class="panel" style="margin-top: 12px;">
    <div class="panel-head"><h2>{$t('questsStats.timelineTitle', { granularity: granularityLabel(granularity, $t) })}</h2></div>
    <LineChart series={completionsSeries} valueFormatter={(v) => formatNumber(v)} />
  </div>

  <div class="panel" style="margin-top: 12px;">
    <div class="panel-head"><h2>{$t('questsStats.topQuestsTitle')}</h2></div>
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th>{$t('questsStats.colQuest')}</th>
            <th>{$t('questsStats.colCompletions')}</th>
          </tr>
        </thead>
        <tbody>
          {#each data.topQuests || [] as row}
            <tr>
              <td>{row.name}</td>
              <td>{formatNumber(row.completions)}</td>
            </tr>
          {:else}
            <tr><td colspan="2" class="muted">{$t('questsStats.noCompletions')}</td></tr>
          {/each}
        </tbody>
      </table>
    </div>
