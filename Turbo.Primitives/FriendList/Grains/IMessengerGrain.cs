...
public partial interface IMessengerGrain : IGrainWithIntegerKey
{
    ...
    Task<List<RelationshipStatusEntrySnapshot>> GetRelationshipStatusSummaryAsync(CancellationToken ct);
    ...
    Task<FriendListErrorCodeType?> SendFriendRequestAsync(PlayerId targetId, string targetName, CancellationToken ct);
    Task ReceiveFriendRequestAsync(PlayerId requesterId, string requesterName, string requesterFigure, CancellationToken ct);
    Task<List<AcceptFriendFailureSnapshot>> AcceptFriendRequestsAsync(List<int> requesterIds, CancellationToken ct);
    ...
    Task SetRelationshipStatusAsync(int friendId, MessengerFriendRelationType relationType, CancellationToken ct);
    ...
    // ── Messaging ────────────────────────────────────────────────────────────
    Task<InstantMessageErrorCodeType?> SendMessageAsync(PlayerId receiverId, string message, int chatId, int confirmationId, CancellationToken ct);
    Task ReceiveMessageAsync(PlayerId senderId, string senderName, string senderFigure, string message, DateTime timestamp, int messageId, CancellationToken ct);
    Task<List<MessageHistoryEntrySnapshot>> GetMessageHistoryAsync(PlayerId friendId, CancellationToken ct);

    // ── Presence notifications ───────────────────────────────────────────────
    Task NotifyOnlineAsync(CancellationToken ct);
    Task NotifyOfflineAsync(CancellationToken ct);
    Task NotifyFriendPresenceChangedAsync(PlayerId friendId, bool online, string figure, string motto, CancellationToken ct);

    // ── Room actions ─────────────────────────────────────────────────────────
    Task ReceiveRoomInviteAsync(PlayerId senderId, string message, CancellationToken ct);

    // ── Search ───────────────────────────────────────────────────────────────
    Task<List<MessengerSearchResultSnapshot>> GetFriendSearchResultsAsync(string query, CancellationToken ct);
}
